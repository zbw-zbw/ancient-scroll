...
export default function Navbar() {
  ...
  return (
    ...
    >
        ...
        <ul className="hidden md:flex items-center gap-8">
          {navItems.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
                className="font-serif text-sm text-light-ink hover:text-cinnabar transition-colors relative group"
              >
                {item.label}
                <span className="absolute -bottom-1 left-0 w-0 h-px bg-cinnabar transition-all duration-300 group-hover:w-full" />
              </Link>
            </li>
          ))}
        </ul>
      ...
      >
        <ul className="flex flex-col items-center gap-4 pb-6 pt-2 bg-xuan/95 backdrop-blur-md border-t border-rule/30">
          {navItems.map((item) => (
            <li key={item.href}>
              <Link
                href={item.href}
