...
export default function ChatInput({
  ...
  return (
    <div className="border-t border-rule/60 bg-xuan px-4 py-3 md:px-6 md:py-4">
      <div className="mx-auto flex max-w-[900px] items-end gap-2 md:gap-3">
        <textarea
          ...
          placeholder="请输入你的问题……"
          disabled={disabled}
          className="max-h-28 flex-1 resize-none rounded-2xl border border-rule bg-surface px-4 py-3 font-serif text-sm text-ink placeholder:text-muted focus:border-cinnabar/50 focus:outline-none disabled:opacity-60"
        />
        <button
          onClick={() => {
            if (value.trim() && !disabled) {
