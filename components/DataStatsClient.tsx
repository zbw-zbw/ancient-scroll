...
export interface DataStatsCounts {
  chapters: number;
  sentences: number;
  beasts: number;
  poems: number;
...
export default function DataStatsClient({ counts }: { counts: DataStatsCounts }) {
  ...
  // Animated counts
  const sentenceCount = useCountUp(counts.sentences, 1200, inView);
  const beastCount = useCountUp(counts.beasts, 1000, inView);
  const characterCount = useCountUp(counts.characters, 1000, inView);
  const poemCount = useCountUp(counts.poems, 1000, inView);
  const quizCount = useCountUp(counts.quizQuestions, 1000, inView);
  const voiceCount = useCountUp(counts.voices, 800, inView);
      ...
      <div className="relative mx-auto max-w-[1100px] px-6">
        ...
        {rate > 0 && (
          <div className="mb-8 grid grid-cols-3 gap-4 sm:grid-cols-6">
            {[
              { value: personalStats.readChapters, total: counts.chapters, label: "已读篇章", unit: "篇" },
              { value: personalStats.collectedBeasts, total: counts.beasts, label: "已集异兽", unit: "只" },
              { value: personalStats.completedPoems, total: counts.poems, label: "已赏诗词", unit: "首" },
              { value: personalStats.dialogueCharacters, total: counts.characters, label: "已对话", unit: "位" },
