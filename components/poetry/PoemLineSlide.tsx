...
export default function PoemLineSlide({ line, active, coverImage, reciting, onRecite }: PoemLineSlideProps) {
  ...
  return (
    ...
    >
      ...
      {/* Strong gradient overlay for text readability — 强度随文字颜色动态调整 */}
      <div
        className="absolute inset-0"
        style={{ backgroundColor: textLight ? "rgba(0,0,0,0.45)" : "rgba(0,0,0,0.2)" }}
      />
      <div
        className="absolute inset-0"
        style={{
          background: textLight
            ...
            : "linear-gradient(to bottom, rgba(0,0,0,0.15) 0%, transparent 25%, transparent 75%, rgba(0,0,0,0.2) 100%)",
        }}
      />
      ...
      <div className="relative z-10 mx-auto w-full max-w-5xl px-4 text-center break-words md:max-w-6xl md:px-6">
        {/* 文字内容卡片底衬 — 毛玻璃效果增强可读性，同时保留背景意境 */}
        <div
          className="mx-auto rounded-2xl px-6 py-8 backdrop-blur-sm md:px-10 md:py-10"
          style={{
            backgroundColor: textLight ? "rgba(0,0,0,0.15)" : "rgba(255,255,255,0.1)",
          }}
        >
          ...
          <h3
            ...
            style={{
              ...
              textShadow: textLight
                ? "0 2px 4px rgba(0,0,0,0.8), 0 4px 16px rgba(0,0,0,0.5), 0 8px 40px rgba(0,0,0,0.3)"
                : "0 1px 2px rgba(255,255,255,0.3), 0 2px 12px rgba(0,0,0,0.2)",
              WebkitTextStroke: textLight ? "0.5px rgba(0,0,0,0.3)" : "none",
            }}
          >
            {line.text}
          </h3>
          ...
          <p
            ...
            style={{
              ...
              textShadow: textLight
                ? "0 1px 4px rgba(0,0,0,0.6), 0 2px 12px rgba(0,0,0,0.4)"
          ...
          {onRecite && (
            ...
            >
              <button
                onClick={onRecite}
                className={`inline-flex items-center gap-1.5 rounded-full px-4 py-1.5 font-serif text-xs backdrop-blur-sm transition-all hover:gap-2 active:scale-95 md:text-sm ${
                  reciting
